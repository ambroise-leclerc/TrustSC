...
use trustsc::verify_scenario::run_scenario;

#[test]
fn every_compiled_scenario_passes() {
    let screen = medui_screen::screen();
    let standard = trustsc::default_standard_text_package().expect("standard text package");
    let displays = trustsc::default_display_text_packages().expect("display text packages");

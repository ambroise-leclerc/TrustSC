# SOUP list and justification — MduX-rust
...
- **Owner:** MduX-rust text authoring maintainers (per the register's `owner` field)
...
device artifact: `fontdue` (glyph rasterization, `tools/mdux-font-baker`), `serde` + `serde_json`
(bake-recipe and evidence `package.json`/`report.json` I/O across `mdux-font-baker`,
`mdux-build`, `mdux-ml-baker`, `mdux-shader-baker`), `sha2` (evidence digests), `toml` (manifest and
recipe parsing), `shaderc` (GLSL→SPIR-V compilation in `tools/mdux-shader-baker`).
...
A manufacturer shipping a device built on MduX-rust should maintain their own SOUP-anomaly tracking

...
- **Scope note:** this file documents how TrustSC's `trustsc-governance` types and design mechanisms
  support a manufacturer's own ISO 14971 risk management file for a device built on it — it is not a
  risk management file for a specific finished device, since TrustSC has no clinical intended use
  of its own.
...
`trustsc_core::DeviceContext.safety_class` (`Class B`/`Class C`) is the top-level classification driving
how much of `trustsc-governance`'s enforcement applies: a `Class C` device's `ComplianceProgram` must
...
`trustsc_governance::Hazard { id, description, controlled_by }` records the outcome of a manufacturer's
...
Not automated by `trustsc-governance` — deciding whether an estimated risk is acceptable as-is is the
manufacturer's clinical/regulatory judgment. `trustsc-governance` records the *outcome* of that judgment
...
The `trustsc_governance::AuditEvent` trail (`Lifecycle`/`Verification` categories, sequenced) gives a
...
`trustsc_governance::ProblemReport { id, summary, closed }` is where field information (via a
manufacturer's own complaint/incident intake) would be recorded once triaged into TrustSC's

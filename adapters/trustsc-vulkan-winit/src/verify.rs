...
//! pure check engine `trustsc-ui-verify` (§2/§3), and the scenario plane (§4) into [`crate::App`] as
//! a verification run mode symmetric with `--headless-smoke`. Per locale: render the compiled
//! screen's static truth offscreen, run every `trustsc-ui-verify` check against it, replay every
...
use trustsc::input::FrameEvents;
use trustsc::realtime::{FrameInputs, ScreenBindings};
use trustsc::screen_text::ScreenTextLayout;
use trustsc::verify_scenario::{ScenarioScript, ScenarioTrace, run_scenario};
use trustsc_ui_verify::{
...
pub enum LocaleSelection {
    ...
    /// ([`trustsc::TextPackage::locales`]) — added automatically as new locales are baked in.
    All,
...
pub(crate) fn run_verify(
    ...
    let standard_package = trustsc::default_standard_text_package()?;
    let display_packages = trustsc::default_display_text_packages()?;
    let image_packages = trustsc::default_image_packages()?;
    ...
    for locale in &resolved_locales {
        ...
        let report_trace_rows: Vec<trustsc_ui_verify::TraceRow> = trace_rows_gov
            .iter()
            .map(|gov_row| {
...
mod tests {
    ...
    fn baseline_round_trip_bootstraps_then_loads() {
        let dir = std::env::temp_dir().join(unique_temp_name("trustsc-verify-baseline-test"));
        let _ = fs::remove_dir_all(&dir);
        let path = dir.join("en-US.txt");
        let checks = vec![CheckResult {
            check_id: "sedation-index::color_hash".to_string(),
            node_id: "sedation-index".to_string(),
            kind: trustsc_ui_verify::CheckKind::ColorHash,
            requirement_id: None,
    ...
    fn missing_baseline_file_loads_as_empty_not_an_error() {
        let path = std::env::temp_dir()
            .join(unique_temp_name("trustsc-verify-baseline-does-not-exist"))
            .with_extension("txt");

...
//! `mdux-font-baker`'s bake/verify evidence contract: a TOML recipe points at a governed
...
mod tests {
    ...
    fn bakes_and_verifies_the_acme_logo_fixture() {
        let recipe = workspace_root().join("tools/mdux-image-baker/fixtures/acme-logo.toml");

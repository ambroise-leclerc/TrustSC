//! `trustsc-docs-lint check` — machine-checks the regulatory citation contract (ADR-020).
//!
...
fn main() -> ExitCode {
    ...
    let root = match args.as_slice() {
        ...
        _ => {
            eprintln!("usage: trustsc-docs-lint check [repo-root]");
    ...
    let mut linter = match Linter::new(&root) {
        Ok(linter) => linter,
        Err(message) => {
            eprintln!("trustsc-docs-lint: {message}");
    ...
    println!(
        "trustsc-docs-lint: {} files, {} citation keys, {} justification blocks checked",
        stats.files, stats.citations, stats.justifications
    );
    if linter.violations.is_empty() {
        println!("trustsc-docs-lint: no violations");
        ExitCode::SUCCESS
    } else {
        for violation in &linter.violations {
            eprintln!("{violation}");
        }
        eprintln!("trustsc-docs-lint: {} violation(s)", linter.violations.len());
        ExitCode::FAILURE
    }
}

/// The workspace root, two levels above this crate's manifest — keeps `cargo run -p
/// trustsc-docs-lint` working from any current directory, matching the baker tools' spirit.

...
mod tests {
    ...
    fn target_output_paths() -> (PathBuf, PathBuf) {
        let base =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../target/trustsc-font-baker-tests");
        (
    ...
    fn find_workspace_root_skips_missing_manifests() {
        let root = temp_dir("trustsc-font-baker-workspace");
        let nested = root.join("tools/trustsc-font-baker/fixtures");
    ...
    fn relative_to_workspace_or_self_normalizes_dotdot_segments() {
        let root = temp_dir("trustsc-font-baker-relative");
        let font_dir = root.join("assets/fonts/roboto");
        let recipe_dir = root.join("tools/trustsc-font-baker/fixtures");
    ...
    fn write_bytes_accepts_plain_filename_outputs() {
        ...
        let root = temp_dir("trustsc-font-baker-write-bytes");

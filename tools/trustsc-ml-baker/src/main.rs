...
use trustsc_ml_baker::{CliInvocation, bake, import, verify};
...
fn usage() -> String {
    "usage: trustsc-ml-baker <bake|verify> <recipe.toml> <package.json> <report.json>\n       trustsc-ml-baker import <model.safetensors> <tensor-map.toml>   (offline authoring only, never CI)".to_string()
}

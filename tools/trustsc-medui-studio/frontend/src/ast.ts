...
export function isDraggable(node: NodeDefinitionDto): boolean {
  return node.position !== null && node.width.kind === "Px" && node.height.kind === "Px";
}

export function dimensionPx(dimension: DimensionDto): number | null {
...
export function isSyntheticPanel(kind: NodeKindDto): boolean {
...
export function rowIdFromPanelId(panelId: string): string {
...
export function findNode(screen: ScreenDefinitionDto, nodeId: string): NodeDefinitionDto | null {
...
export function updateNode(
  ...
  nodeId: string,
  updater: (node: NodeDefinitionDto) => NodeDefinitionDto,
): ScreenDefinitionDto {
  let changed = false;
...
export function collectIds(screen: ScreenDefinitionDto): Set<string> {
...
export function generateNodeId(screen: ScreenDefinitionDto, kindName: string): string {
...
export function appendNode(screen: ScreenDefinitionDto, node: NodeDefinitionDto): ScreenDefinitionDto {
...
export function removeNode(screen: ScreenDefinitionDto, nodeId: string): ScreenDefinitionDto {
  let changed = false;
  const items: ScreenItemDto[] = [];
  for (const item of screen.items) {
    if (item.type === "Component" && item.id === nodeId) {
      changed = true;
      continue;
...
export function snap(value: number, grid: number, disabled: boolean): number {
...
export function proposedBounds(node: NodeDefinitionDto): Bounds | null {
 ...
 * serialized. */
export function convertToAbsolute(screen: ScreenDefinitionDto, nodeId: string, bounds: Bounds): ScreenDefinitionDto {

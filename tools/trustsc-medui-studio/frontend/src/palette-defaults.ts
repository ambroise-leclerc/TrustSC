...
import type { NodeDefinitionDto, NodeKindDto, Palette, TextKeyInfo } from "./api.js";
...
function sizeForTextKey(info: TextKeyInfo, minW: number, minH: number): [number, number] {
  let width = 0;
  let height = 0;
...
export function defaultTextKey(palette: Palette): TextKeyInfo | undefined {
  let best: TextKeyInfo | undefined;
  let bestCoverage = -1;
...
export function cannotCreateReason(kindName: string, palette: Palette): string | null {
...
export function defaultNodeAt(
  kindName: string,
  palette: Palette,
  id: string,
  position: [number, number],
): NodeDefinitionDto | null {

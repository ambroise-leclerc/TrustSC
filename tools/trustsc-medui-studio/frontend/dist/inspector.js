...
export class Inspector {
    constructor(container, palette, editor) {
    ...
    renderGlobal() {
        ...
        const children = [
            ...
            this.field("Layout spacing (px)", this.numberInput(screen.layout.spacing, 0, (value) => this.editor.applyScreenUpdate((s) => ({ ...s, layout: { ...s.layout, spacing: value } })))),
            this.field("Layout padding (px)", this.numberInput(screen.layout.padding, 0, (value) => this.editor.applyScreenUpdate((s) => ({ ...s, layout: { ...s.layout, padding: value } })))),
    ...
    renderNode(nodeId) {
        ...
        children.push(this.idField(node.id, (newId) => {
        ...
        for (const dimKey of ["width", "height"]) {
            ...
            children.push(this.field(dimKey === "width" ? "Width" : "Height", this.dimensionInput(node[dimKey], fillAllowed, positioned, (dim) => this.editor.applyNodeUpdate(this.targetNodeId(), (n) => ({ ...n, [dimKey]: dim })))));
        }
        if (node.position !== null) {
            children.push(this.positionField(node.position));
        }
        else {
            children.push(el("p", { class: "inspector__hint" }, [
    ...
    renderRow(rowId) {
        ...
        const children = [
            ...
            this.field("Height", this.dimensionInput(row.height, true, false, (dim) => this.editor.applyRowUpdate(this.targetRowId(), (r) => ({ ...r, height: dim })))),
            this.field("Spacing (px)", this.numberInput(row.spacing, 0, (value) => this.editor.applyRowUpdate(this.targetRowId(), (r) => ({ ...r, spacing: value })))),
            this.field("Background", this.colorSelect(row.background, true, (token) => this.editor.applyRowUpdate(this.targetRowId(), (r) => ({ ...r, background: token })))),
        ];
        this.container.replaceChildren(...children);
    }
    ...
    kindPropFields(node, schema) {
        ...
        for (const prop of schema.properties) {
            ...
            const apply = (value) => this.editor.applyNodeUpdate(this.targetNodeId(), (n) => ({
                ...n,
            ...
            switch (prop.domain.kind) {
                ...
                case "SystemEvent":
                    ...
                    break;
                case "ClockFormat":
                    fields.push(this.field(prop.key, this.enumSelect(["TimeSeconds", "DateTimeSeconds"], String(current ?? ""), apply)));
                    break;
                case "Charset":
                    // One approved charset (AsciiText -> SET-ASCII-TEXT); shown for transparency, not
                    // (yet) a real choice.
                    fields.push(this.field(prop.key, this.enumSelect(["AsciiText"], "AsciiText", () => undefined, true)));
                    break;
                case "MaxLength":
    ...
    numberInput(current, min, onCommit) {
    ...
    colorSelect(current, allowNone, onPick) {
        const select = el("select", { class: "inspector__control" });
        if (allowNone) {
    ...
    textKeyField(label, current, nodeWidthPx, onPick) {
        const overBudget = (info) => nodeWidthPx !== null && info.entries.some((entry) => entry.width_px > nodeWidthPx);
        const select = el("select", { class: "inspector__control" });
        ...
        const renderDetails = () => {
            ...
            details.replaceChildren(...(info?.entries ?? []).map((entry) => {
        ...
        const renderOptions = () => {
            ...
            select.replaceChildren(...this.palette.text_keys
    ...
    templateSelect(current, onPick) {
        const ids = [...new Set(this.palette.templates.map((template) => template.id))];
        return this.enumSelect(ids, current, onPick);
    }
    positionField(position) {
        const commit = (axis, value) => this.editor.applyNodeUpdate(this.targetNodeId(), (n) => {
            const next = n.position ? [...n.position] : [0, 0];
            next[axis] = value;
    ...
    dimensionInput(dim, fillAllowed, positioned, onCommit) {
        const px = el("input", {
            type: "number",
            class: "inspector__control",
            min: "1",
    ...
    statesField(node) {
        ...
        const applyLists = (nextStates, nextColors) => {
            const nodeId = this.targetNodeId();
            this.editor.applyNodeUpdate(nodeId, (n) => ({
                ...n,
        ...
        const rows = states.map((stateKey, index) => {
            const keySelect = this.enumSelect(this.palette.text_keys.map((info) => info.string_id), stateKey, (value) => {
                const next = [...states];
                next[index] = value;
            ...
            remove.addEventListener("click", () => {
                applyLists(states.filter((_, i) => i !== index), colors.filter((_, i) => i !== index));
            });
            if (states.length === 1) {

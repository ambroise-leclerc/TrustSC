...
MduX-rust is a software development kit, not a finished medical device — clause 1's scope applies
to a manufacturer's *use* of MduX-rust inside their own device software, not to MduX-rust as a
shrink-wrapped product. `docs/regulatory-compliance.md` states this explicitly: MduX-rust provides
...
Two definitions matter most for how this corpus and `mdux-governance` are structured:

- **SOUP** (Software Of Unknown Provenance) — software not developed to comply with IEC 62304 for
  the device at hand. MduX-rust tracks SOUP explicitly in `docs/governance/soup-register.toml`,
...
A manufacturer's software development process runs inside their ISO 13485 QMS. MduX-rust does not
...
overall ISO 14971 risk management process — the two are not separate activities. `mdux-governance`'s
...
`mdux_core::SafetyClass` models only `B` and `C` — MduX-rust's UI runtime, ML inference engine, and
...
component is genuinely Class A does not need `mdux-governance`'s enforcement and can use a lighter
process; nothing in this corpus should be read as implying MduX-rust supports or was validated for
...
`ComplianceProgram::validate()` (`crates/mdux-governance/src/lib.rs`) enforces one classification
...
Not applicable to MduX-rust today — the project has no shipped, previously-unclassified software
line being brought under IEC 62304 retroactively. A manufacturer integrating an older UI or ML
component alongside MduX-rust would apply §4.4's gap-analysis provisions to that component, not to
MduX-rust itself.

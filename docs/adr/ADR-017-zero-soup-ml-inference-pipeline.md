...
Manufacturers building on TrustSC want to embed learned models — starting with signal-classification
...
byte-verified evidence" pattern (`tools/trustsc-font-baker` → `generated/fonts/...`). This ADR applies both
to machine-learning inference.

## Decision

1. **Split the ML subsystem into three governed crates, mirroring the text pipeline (ADR-001):**
   - `trustsc-ml-schema` — the shared contract: `ModelPackage`, layer/tensor types, golden self-test
     vectors, and a `DeterminismEvidence` fingerprint. No I/O, no parsing logic. Depends only on
     `trustsc-core`.
   - `trustsc-ml-authoring` — host-side: imports weights (safetensors, hand-parsed — see point 6),
   ...
   - `trustsc-ml-runtime` — device-side: an immutable-package inference engine (`Classifier1D`) doing only
...
2. **Weights are data, not code.** A `ModelPackage` is authored once, offline, by
   `tools/trustsc-ml-baker` (mirroring `tools/trustsc-font-baker`) and committed as evidence under
   ...
   (`trustsc_build::ModelPackage` + `trustsc::include_model!()`), the same JSON-to-Rust codegen path already
   ...
   regenerating `package.json` — zero change to `trustsc-ml-runtime` or to application source.** This is
...
3. **Determinism is enforced by strictly-ordered scalar arithmetic, not merely by pinning inputs.**
   Every kernel in `trustsc-ml-runtime` accumulates in a single, fixed, documented order (e.g. Conv1D sums
   ...
   than the equivalent scalar sequence. This is what lets `trustsc-ml-authoring` compute "golden" reference
   outputs on the host and have `trustsc-ml-runtime` reproduce them bit-for-bit on the device.
...
5. **v1 scope is `f32` tensors and a small kernel set: `Dense`, `Conv1D`, `MaxPool1D`/`AvgPool1D`,
   ...
   ADR; `trustsc-ml-schema` rejects any package declaring a dtype other than `F32`.
6. **Weight import avoids adding runtime-adjacent SOUP.** The safetensors format is a `u64` header
   ...
   `trustsc-ml-authoring` hand-parses this directly using the already-registered `serde_json`, rather than

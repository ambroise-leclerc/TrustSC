...
- Host-only bake helpers such as `tools/trustsc-font-baker` may use separately reviewed SOUP crates, but those dependencies remain outside the runtime boundary; only the reviewed asset manifests and generated package bytes cross into release evidence.

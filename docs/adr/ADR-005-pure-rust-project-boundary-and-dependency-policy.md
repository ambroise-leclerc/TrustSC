...
- The governed workspace crates (`trustsc-core`, `trustsc-governance`, `trustsc-ui`, `trustsc`, `trustsc-text-schema`, `trustsc-text-authoring`, and `trustsc-text-runtime`) are pure Rust implementation boundaries with safe Rust public APIs and no C/C++ source or bindgen-generated interfaces.

...
| [ADR-020](ADR-020-ai-interaction-standardization.md) | AI interaction standardization | Committed `AGENTS.md` as the canonical agent instruction file, Agent Skills under `.claude/skills/`, the `trustsc-docs-lint` citation linter in CI, and the no-MCP-servers policy. |
...
| [ADR-022](ADR-022-medui-studio-hosted-designer.md) | MedUI Studio — hosted interactive designer | A hosted web service (not native/local/Figma) lets a manager design `.medui` screens without a developer; reuses the compiler and `OffscreenRenderer` in-process, saves via a propose-change PR flow through the unchanged CI gate. |

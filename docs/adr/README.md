...
| [ADR-021](ADR-021-repository-migration-to-trustsc.md) | Repository migration to TrustSC | New repository, byte-identical history (no rewrites, no backdating), old repository archived; the `trustsc-*` → `trustsc-*` rename follows as separate PRs. |

...
hello-world screen; every windowed MduX application would need to copy it verbatim.
...
1. **`adapters/` is a third trust-zone directory**, alongside `crates/` (governed, `#![forbid(unsafe_code)]`,
   ...
   function must take or return owned Rust data already defined by a governed crate (e.g. `mdux::Framework`,
   `mdux::CompiledScreenPackage`, `mdux::screen_text::ScreenTextLayout`). No `ash::vk` handle, `winit`
...
2. **The first adapter crate is named `mdux-vulkan-winit`**, at `adapters/mdux-vulkan-winit`. The name
...
4. **SOUP register scope widens to cover presentation-adapter dependencies.** `ash`, `ash-window`, and
   ...
   `adapters/mdux-vulkan-winit` and ship in any binary that links it); `winit` likewise. `shaderc` is
   registered as tools-only, exactly like the font-baker's `fontdue`/`toml` entries, since it never
   compiles into `adapters/mdux-vulkan-winit` or any application.

## Consequences

- The reusable renderer can be extracted from `examples/hello_world` into `adapters/mdux-vulkan-winit`

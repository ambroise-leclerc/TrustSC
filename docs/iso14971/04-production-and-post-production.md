...
post-market clause — TrustSC, as a library rather than a fielded device, has no production or
post-production activity of its own; every mechanism cited below is scoped to *what a manufacturer's
own post-market process could draw on from a device built on TrustSC*, not to anything TrustSC
performs itself.
...
entire lifecycle — not only at initial release. TrustSC has no post-market presence: it is
...
and post-market surveillance activities, none of which TrustSC performs on their behalf.
...
regulatory or standards developments. `trustsc_governance::ProblemReport { id, summary, closed }`
(`crates/trustsc-governance/src/lib.rs`) is the nearest TrustSC type to an information-collection
...
it is scoped to defects found in TrustSC's own development or a manufacturer's integration testing,
...
device built on TrustSC, `Hazard`/`Requirement`/`VerificationCase`'s existing linkage —
...
through `trustsc-governance`'s ordinary requirement/verification machinery exactly as any other risk

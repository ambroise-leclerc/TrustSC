...
§7 is where ISO 13485 is most relevant to MduX-rust's actual engineering practice. It covers
...
MduX-rust's own mechanisms genuinely line up — which is mostly in §7.3 — this module says so
...
For a software-only or software-dominant product built on MduX-rust, this planning activity maps
onto a manufacturer choosing which `mdux-core`/`mdux-governance` types to populate and how: a
`DeviceContext` (`crates/mdux-core/src/lib.rs`) fixing the product's identity and `SafetyClass`, a
...
Vulkan SC (Class C). `FrameworkBuilder::build()` (`crates/mdux/src/lib.rs`) refuses to construct a
...
MduX-rust plays no role in — it has no customers of its own in the regulatory sense, no
delivery/post-delivery process, and no advisory-notice mechanism. The one loosely-related
observation worth recording: `mdux_governance::Requirement.source_clause` gives a manufacturer a
...
control points for a software item. A manufacturer building on MduX-rust satisfies both by running
...
on MduX-rust can point to for the software-item slice of this plan.
...
`mdux_governance::Requirement { id, title, source_clause, verification_intent }` is the structured
...
For the UI slice of a device, `mdux-ui-dsl-authoring`'s `.medui` compiler output (a generated
...
MduX-rust's own analogue, again scoped to *this project's* design and development rather than a
...
device built on MduX-rust are a separate, broader activity this project does not perform on the
...
or `ComplianceProgram::validate()` fails (`crates/mdux-governance/src/lib.rs`) — the same rule
...
MduX-rust provides evidence toward *engineering-level* validation of the software components it
...
For a device built on MduX-rust, "manufacturing" for the software item is closer to "the build and
...
`mdux_governance::AuditEvent`'s `Lifecycle` category records every requirement/hazard/verification
...
MduX-rust's own dependency governance is the closest analogue an incorporating manufacturer will
...
tells a manufacturer's own §7.4 purchasing-control process what MduX-rust's transitive dependency
surface looks like and how each entry is risk-controlled — but a manufacturer treating MduX-rust
itself as purchased/outsourced software still needs their own §7.4.1 supplier evaluation of
MduX-rust as a supplier, which this project cannot perform on the manufacturer's behalf.
...
software library. MduX-rust is not manufactured, packaged, installed, or serviced in the sense §7.5
describes; a device incorporating MduX-rust applies the whole of §7.5 to its own physical/production
...
(`crates/mdux-core/src/lib.rs`), formatting `"{product_name} {version} ({safety_class})"` — is a
...
against a specification), that tool itself needs validation before its output is trusted. MduX-rust's
...
never re-checking. `tools/mdux-font-baker`, `mdux-shader-baker`, and `mdux-ml-baker`'s `verify`
...
regulated release with, rather than relying on MduX-rust's own CI configuration for that guarantee.

...
about where MduX-rust simply has no analogue and the manufacturer's own QMS carries the whole
...
This is an organizational-leadership obligation with no software analogue — MduX-rust cannot
provide "evidence of management commitment" for an organization it has no relationship with. The
closest indirect signal available to an organization evaluating whether to adopt MduX-rust is the
...
MduX-rust's contribution here is narrow but real: `mdux_governance::Requirement { id, title,
...
Entirely an organizational document; MduX-rust has no quality policy of its own to point to, and a
...
`ComplianceProgram::release_evidence_summary()` (`crates/mdux-governance/src/lib.rs`) — a one-line
...
their review are the organization's to set, not MduX-rust's to supply.
...
No MduX-rust analogue at the QMS-planning level. The closest process-level parallel within this
...
Organizational roles with no engineering analogue. MduX-rust's own repository has commit review and
...
communication takes place regarding QMS effectiveness. `mdux_governance::AuditEvent`'s `Lifecycle`
...
management review that covers the software slice of a device built on MduX-rust — they give a
...
date. They are inputs only: MduX-rust performs no review itself, sets no review interval, and
...
organizational resourcing decision — outside MduX-rust's scope entirely.
...
MduX-rust's contribution is indirect but genuine: the trust-zone architecture (ADR-005) narrows
...
MduX-rust is process equipment (software) in this sub-clause's sense when a manufacturer builds it
into their toolchain. `CLAUDE.md`'s "Replaying CI locally" section, and `.github/workflows/ci.yml`
itself, are the closest thing MduX-rust offers to a documented, reproducible build infrastructure
...
Neither sub-clause has any MduX-rust analogue: this is a software library with no physical work
environment, personnel health/clothing requirements, or contamination exposure of its own. A
manufacturer producing a device with physical components alongside MduX-rust-based software applies

...
This folder is an LLM- and human-navigable breakdown of IEC 62366-1, TrustSC's usability
...
TrustSC's MedUI DSL and `--verify-ui` tooling provide structural, compile-time and inspection-based
...
- [`schemas/usability-engineering-record.schema.json`](schemas/usability-engineering-record.schema.json) —
  JSON Schema for one use-scenario/evaluation entry, documentation-level (no `trustsc-governance` Rust
  type mirrors it yet).
...
- `software_development_file/regulatory/IEC_62366/` (added in a later PR in this stack) —
  TrustSC's own filled-in Usability Engineering File, which cites into this corpus.
...
be explicit about the boundary between what TrustSC's MedUI DSL/`--verify-ui` mechanically enforces

...
- **§1 Scope** — applies to use-related safety of a device's user interface, not just correct
  function; MduX-rust narrows a small, structural slice of this (text/layout/safety-critical
...
- **§4.1 Application of this standard** — how the process applies to a device UI as a whole, and
  the user-interface-of-unknown-provenance (legacy UI) gap-analysis path, which MduX-rust has no
...
- **§5.2 Identify frequently used functions and hazard-related use scenarios** — a clinical-workflow
  analysis MduX-rust cannot run; `Hazard.controlled_by` and the usability-engineering-record schema
...
- **§5.7 Summative evaluation** — MduX-rust provides no mechanism for this clause; the governance

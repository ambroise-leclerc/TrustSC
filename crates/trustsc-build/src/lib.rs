...
//! Build-script helper that wraps the MedUI DSL compiler (`trustsc-ui-dsl-authoring`) behind a
...
//! ```
//!
//! Pair this with `trustsc::include_medui_screen!()` in the crate's `src/` to bring the generated
...
use trustsc_ui_dsl_authoring::{CompileOptions, ImagePackages, TextPackages};
...
impl MeduiScreen {
    ...
    /// `$OUT_DIR/trustsc_medui_screen.rs`, and emits `cargo:rerun-if-changed` for the source file.
    pub fn compile(self) -> Result<(), DynError> {
        ...
        let generated_path = out_dir.join("trustsc_medui_screen.rs");
        ...
        trustsc_ui_dsl_authoring::compile_medui_file_to_rust_module(
            &medui_path,
            &generated_path,

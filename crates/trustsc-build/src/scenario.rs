...
//! `trustsc::include_scenarios!()` brings into scope — the same authored-source-in, static-data-out
...
//! ```
//!
//! Pair this with `trustsc::include_scenarios!()` in the crate's `src/` to bring the generated
...
/// Rust module consumed by `trustsc::include_scenarios!()`.
...
impl ScenarioSet {
    ...
    /// `$OUT_DIR/trustsc_scenarios.rs`, and emits `cargo:rerun-if-changed` for the directory and
    ...
    pub fn compile(self) -> Result<(), DynError> {
        ...
        let generated_path = out_dir.join("trustsc_scenarios.rs");
...
/// `trustsc::verify_scenario::ScenarioScript`, one field at a time, as owned data.
...
/// Mirrors `trustsc::WidgetEvent` exactly (ADR-015): every variant that can arrive from scripted
...
fn emit_scenarios_module(scenarios: &[CompiledScenario]) -> String {
    ...
    let _ = writeln!(
        output,
        "pub static SCENARIOS: &[::trustsc::verify_scenario::ScenarioScript] = &["
    );
    for scenario in scenarios {
...
fn emit_scenario(output: &mut impl fmt::Write, scenario: &CompiledScenario) {
    let _ = writeln!(output, "    ::trustsc::verify_scenario::ScenarioScript {{");
    let _ = writeln!(output, "        id: {:?},", scenario.id);
    let _ = writeln!(output, "        requirement_ids: {},", emit_str_slice(&scenario.requirement_ids));
    let _ = writeln!(
        output,
        "        clock: ::trustsc::verify_scenario::ScenarioClock {{ year: {}, month: {}, day: {}, hours: {}, minutes: {}, seconds: {} }},",
        scenario.clock.year,
...
fn emit_step(step: &CompiledStep) -> String {
    match step {
        CompiledStep::Event(event) => {
            format!("::trustsc::verify_scenario::ScenarioStep::Event({})", emit_event(event))
        }
        CompiledStep::Advance { frames } => {
            format!("::trustsc::verify_scenario::ScenarioStep::Advance {{ frames: {frames} }}")
        }
        CompiledStep::ExpectText { source, value } => format!(
            "::trustsc::verify_scenario::ScenarioStep::ExpectText {{ source: {source:?}, value: {value:?} }}"
        ),
        CompiledStep::ExpectStatus { source, value } => format!(
            "::trustsc::verify_scenario::ScenarioStep::ExpectStatus {{ source: {source:?}, value: {value} }}"
        ),
        CompiledStep::ExpectNumber { source, value } => format!(
            "::trustsc::verify_scenario::ScenarioStep::ExpectNumber {{ source: {source:?}, value: {value} }}"
        ),
        CompiledStep::Capture { label } => {
            format!("::trustsc::verify_scenario::ScenarioStep::Capture {{ label: {label:?} }}")
        }
    }
}
...
fn emit_event(event: &CompiledEvent) -> String {
    match event {
        CompiledEvent::ButtonPressed { source } => {
            format!("::trustsc::WidgetEvent::ButtonPressed {{ source: {source:?} }}")
        }
        CompiledEvent::CharTyped { source, character } => format!(
            "::trustsc::WidgetEvent::CharTyped {{ source: {source:?}, character: {character:?} }}"
        ),
        CompiledEvent::Backspace { source } => {
            format!("::trustsc::WidgetEvent::Backspace {{ source: {source:?} }}")
        }
        CompiledEvent::Delete { source } => {
            format!("::trustsc::WidgetEvent::Delete {{ source: {source:?} }}")
        }
        CompiledEvent::CaretMoved { source, position } => format!(
            "::trustsc::WidgetEvent::CaretMoved {{ source: {source:?}, position: {position} }}"
        ),
        CompiledEvent::TextCommitted { source } => {
            format!("::trustsc::WidgetEvent::TextCommitted {{ source: {source:?} }}")
        }
        CompiledEvent::FocusChanged { source } => format!(
            "::trustsc::WidgetEvent::FocusChanged {{ source: {} }}",

...
//! ```
//!
//! Pair this with `trustsc::include_model!()` in the crate's `src/` to bring the generated
//! `medui_model` module into scope, exposing `medui_model::model() -> trustsc::ModelPackage`.
...
/// consumed by `trustsc::include_model!()`.
...
impl ModelPackage {
    ...
    /// Parses and re-emits the committed model package as `$OUT_DIR/trustsc_ml_model.rs`, and
    /// emits `cargo:rerun-if-changed` for the source file. Does not re-validate or re-bake —
    /// that already happened when `tools/trustsc-ml-baker` produced (and CI verified) the
    ...
    pub fn compile(self) -> Result<(), DynError> {
        ...
        let generated_path = out_dir.join("trustsc_ml_model.rs");
...
/// Parses a model package document and renders it into `$OUT_DIR/trustsc_ml_model.rs` source, or
...
fn render_model_package(document: &ModelPackageDocument) -> String {
    ...
    let _ = writeln!(out, "pub fn model() -> ::trustsc::ModelPackage {{");
    let _ = writeln!(out, "    ::trustsc::ModelPackage {{");
    let _ = writeln!(out, "        model_id: {},", rust_string(&document.model_id));
    let _ = writeln!(out, "        dtype: ::trustsc::MlDtype::F32,");
    let _ = writeln!(
        out,
        "        input_spec: ::trustsc::MlInputSpec {{ length: {}, channels: {} }},",
        document.input_spec.length, document.input_spec.channels
    );
    let _ = writeln!(
        out,
        "        output_spec: ::trustsc::MlOutputSpec {{ classes: {}, labels: vec![{}] }},",
    ...
    for tensor in &document.tensors {
        let _ = writeln!(
            out,
            "            ::trustsc::MlTensor {{ id: {}, shape: vec![{}], data: vec![{}] }},",
    ...
    for golden in &document.golden_vectors {
        let _ = writeln!(
            out,
            "            ::trustsc::MlGoldenVector {{ input: vec![{}], expected: vec![{}] }},",
    ...
    let _ = writeln!(
        out,
        "        evidence: ::trustsc::MlDeterminismEvidence {{ package_sha256: {}, toolchain_id: {}, build_recipe_sha256: {}, weights_source_sha256: {} }},",
...
fn render_layer(layer: &LayerDocument) -> String {
    match layer {
        LayerDocument::Conv1d {
            ...
            "::trustsc::MlLayer::Conv1D {{ weights_id: {}, bias_id: {}, in_channels: {in_channels}, out_channels: {out_channels}, kernel: {kernel}, stride: {stride}, padding: {padding} }}",
            rust_string(weights_id),
            rust_string(bias_id),
        ),
        LayerDocument::MaxPool1d { window, stride } => {
            format!("::trustsc::MlLayer::MaxPool1D {{ window: {window}, stride: {stride} }}")
        }
        LayerDocument::AvgPool1d { window, stride } => {
            format!("::trustsc::MlLayer::AvgPool1D {{ window: {window}, stride: {stride} }}")
        }
        LayerDocument::Flatten => "::trustsc::MlLayer::Flatten".to_string(),
        LayerDocument::Dense {
            weights_id,
            bias_id,
            in_features,
            ...
            "::trustsc::MlLayer::Dense {{ weights_id: {}, bias_id: {}, in_features: {in_features}, out_features: {out_features} }}",
            rust_string(weights_id),
            rust_string(bias_id),
        ),
        LayerDocument::Relu => "::trustsc::MlLayer::Relu".to_string(),
        LayerDocument::Sigmoid => "::trustsc::MlLayer::Sigmoid".to_string(),
        LayerDocument::Softmax => "::trustsc::MlLayer::Softmax".to_string(),
    }
}
...
mod tests {
    ...
    /// baked and CI-verified by `tools/trustsc-ml-baker`) exactly as `compile()` does, proving the
    ...
    fn renders_the_committed_eeg_demo_package_into_valid_looking_rust_source() {
        ...
        assert!(rendered.contains("pub fn model() -> ::trustsc::ModelPackage {"));
        assert!(rendered.contains("\"EEG-DOA-LINEAR\".to_string()"));
        assert!(rendered.contains("::trustsc::MlLayer::Flatten"));
        assert!(rendered.contains("::trustsc::MlLayer::Dense {"));
        assert!(rendered.contains("::trustsc::MlLayer::Softmax"));
        assert!(rendered.contains("::trustsc::MlTensor {"));

        let document: ModelPackageDocument =
            serde_json::from_str(&document_text).expect("committed package.json should parse");
        assert_eq!(
            rendered.matches("::trustsc::MlGoldenVector {").count(),

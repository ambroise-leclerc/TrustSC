...
//! `ModelPackage` both `mdux-ml-authoring` (host-side compiler) and `mdux-ml-runtime`
//! (device-side inference engine) agree on. This crate contains no I/O and no arithmetic —
//! only data types and referential-integrity validation, mirroring `mdux-text-schema`.
...
impl ModelPackage {
    ...
    /// the value a `mdux-ml-runtime` const-generic buffer capacity must be sized to hold.

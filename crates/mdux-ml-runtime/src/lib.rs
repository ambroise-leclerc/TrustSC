...
//! immutable `mdux_ml_schema::ModelPackage` and evaluates it with plain, strictly-ordered
...
use mdux_ml_schema::{Layer, ModelPackage};
...
/// shape trace `mdux-ml-schema` uses at compile time, recomputed here so the runtime never
...
impl<'a, const MAX_UNITS: usize, const MAX_OUT: usize> Classifier1D<'a, MAX_UNITS, MAX_OUT> {
    ...
    fn find_tensor(&self, id: &str) -> MduxResult<&'a mdux_ml_schema::Tensor> {
        self.package
            .find_tensor(id)
...
mod tests {
    use super::*;
    use mdux_ml_schema::{DeterminismEvidence, Dtype, GoldenVector, InputSpec, OutputSpec, Tensor};

...
const REPORT_KIND: &str = "trustsc-ui-verification";

/// One scripted-scenario step's trace (ADR-016 §4/§5): the event injected, what was expected,
/// what was observed, and whether it matched. `trustsc-ui-verify` only defines the shape here — the
...
/// device context, the backend identity and the capture file paths), `trustsc-ui-verify` only knows
...
mod tests {
    ...
    fn emits_a_minimal_report_byte_for_byte() {
        let report = VerificationReport {
            tool_name: "trustsc-ui-verify".to_string(),

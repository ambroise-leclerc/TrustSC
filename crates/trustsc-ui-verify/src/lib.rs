...
//! `trustsc-ui-verify`: the pure, dependency-free check engine that gives golden references their
...
use trustsc_ui::{CompiledNodeKind, CompiledScreenPackage, CvCheckKind, Rect};
...
/// produced it.
pub const TOOL_NAME: &str = "trustsc-ui-verify";
...
/// depends on `trustsc-ui`, not the text runtime), and any committed `ColorHash` baseline per node.

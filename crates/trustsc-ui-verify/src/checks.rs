...
use trustsc_ui::{CompiledNode, CompiledNodeKind, CvCheckKind, GoldenReferenceEntry, Rect, resolve_color_token};
...
fn chrome_sampling(node: &CompiledNode) -> Option<(String, [u8; 4], Vec<Rect>)> {
    match node.kind {
        ...
        CompiledNodeKind::TextInput(_) => {
            ...
            // (adapters/trustsc-vulkan-winit/src/renderer.rs `create_interactive_rect_resources`),

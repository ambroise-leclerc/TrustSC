...
//! running the package through the exact same arithmetic `trustsc-ml-runtime` uses on-device —
...
/// I/O happens inside compilation itself, matching `trustsc-text-authoring::TextCompilationInput`.
#[derive(Clone, Debug, PartialEq)]
...
/// layers via `trustsc-ml-runtime`, recording input/output pairs as golden self-test vectors.
